def inputGetFile(String savedfile = null) {
    ...
    if (savedfile == null) {
        def inputFile = input message: 'Upload file', parameters: [file(name: 'library_data_upload'), string(name: 'restaurantCSV', defaultValue: 'My.csv')]
        filedata = inputFile['library_data_upload']
        filename = inputFile['restaurantCSV']
    } else {
